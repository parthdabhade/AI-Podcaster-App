...
const LeftSidebar = () => {
  ...
  const { user } = useUser(); // Fetch logged-in user
  ...
  return (
    ...
    >
      <nav className="flex flex-col gap-6">
        ...
        {/* My Profile Section (Dynamic Profile Link) */}
        {user && (
          <Link
            href={`/profile/${user.id}`} // ✅ Dynamically navigates to correct profile
            className={cn("flex gap-3 items-center py-4 max-lg:px-4 justify-center lg:justify-start", {
              "bg-nav-focus border-r-4 border-orange-1": pathname === `/profile/${user.id}`,
            })}
          >
            <Image src="/icons/profile.svg" alt="Profile" width={24} height={24} />
            <p className="max-lg:hidden">My Profile</p>
          </Link>
        )}
      </nav>
